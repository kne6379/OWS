...
function likePost(a, b) {
    const checkData = JSON.parse(a)
        checkData.includes(b)? checkData.splice(a.indexOf(b),1) : checkData.push(+b)
        a = JSON.stringify(checkData)
        return a
}

likeRouter.post('/:feedId/feedlike', requireAccessToken, async (req, res, next)=> {
    try {
        const { userId } = req.user
        const { feedId } = req.params

        const userData = await prisma.user.findUnique({where: { userId: userId }})
        const feedData = await prisma.feed.findUnique({where: { feedId: +feedId }})
